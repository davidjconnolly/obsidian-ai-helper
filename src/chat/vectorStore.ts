...
export class VectorStore {
  ...
  private index: Map<string, { chunks: NoteChunk[]; maxScore: number }> =
    new Map();
  ...
  > {
    ...
    if (options.query) {
      ...
      logDebug(
        this.settings,
        `Vector search using processed query: ${JSON.stringify({
    ...
    for (const [path, noteEmbedding] of this.embeddings.entries()) {
      ...
      const termsForTitleMatch =
        queryProcessingResult?.expandedTokens || searchTerms;
      ...
      for (let i = 0; i < noteEmbedding.chunks.length; i++) {
        ...
        const phraseMatchScore = this.calculatePhraseMatchScore(
          chunk.content,
          phrasesForMatch,
        );

        // Calculate term match score for expanded terms
        const termMatchScore = this.calculateTermMatchScore(
          chunk.content,
          termsForTitleMatch,
        );
      ...
      noteChunks.sort((a, b) => {
        ...
        const totalScoreA =
          a.similarity + a.phraseMatchScore + a.termMatchScore + headerBoostA;
        const totalScoreB =
          b.similarity + b.phraseMatchScore + b.termMatchScore + headerBoostB;
    ...
    logDebug(
      this.settings,
      `Search results with scores: ${JSON.stringify(
        sortedResults.map((r) => ({
          path: r.path,
  ...
  private calculateCosineSimilarity(a: Float32Array, b: Float32Array): number {
    ...
    if (a.length !== b.length) {
      logError(
        `Cannot calculate similarity between vectors of different dimensions (${a.length} vs ${b.length})`,
      );
      return 0;
    }
  ...
  addEmbedding(path: string, embedding: NoteEmbedding) {
    ...
    if (!embedding || !embedding.chunks || embedding.chunks.length === 0) {
      logDebug(
        this.settings,
        `Skipping embedding for path: ${path} - No chunks available`,
      );
      return;
    }
    ...
    const validChunks = embedding.chunks.every((chunk) => {
      if (!chunk.embedding || chunk.embedding.length !== this.dimensions) {
        logError(
          `Invalid chunk embedding in ${path}. Expected ${this.dimensions} dimensions, got ${chunk.embedding?.length || 0}`,
        );
        return false;
      }
      return true;
    ...
    if (!validChunks) {
      logDebug(
        this.settings,
        `Skipping invalid embedding for path: ${path} - Dimension mismatch`,
      );
      return;
    }
    ...
    logDebug(
      this.settings,
      `Added embedding for ${path} with ${embedding.chunks.length} chunks`,
    );
  ...
  getChunk(path: string, chunkIndex: number): NoteChunk | null {
    ...
    if (
      !noteEmbedding ||
      chunkIndex < 0 ||
      chunkIndex >= noteEmbedding.chunks.length
    ) {
      return null;
    }
   ...
   */
  private calculatePhraseMatchScore(
    content: string,
    phrases: string[],
  ): number {
