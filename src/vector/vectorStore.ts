...
import { NoteEmbedding, SearchResult, VectorStoreOptions } from '../types';

export class VectorStore {
  private embeddings: Map<string, NoteEmbedding> = new Map();
  private dimensions: number;
  private index: Map<string, { chunks: any[], maxScore: number }> = new Map();
  ...
  async search(queryEmbedding: Float32Array, options: VectorStoreOptions): Promise<SearchResult[]> {
  ...
  getChunk(path: string, chunkIndex: number): any | null {
